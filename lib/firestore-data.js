...
const COLLECTIONS = {
  ...
  patientImportantPeople: 'patientImportantPeople',
...
export function subscribeToPatientImportantPeople(patientId, onItems, onError) {
  return subscribeToPatientCollection(
    COLLECTIONS.patientImportantPeople,
    patientId,
    onItems,
    onError,
    (leftItem, rightItem) =>
      Number(rightItem.updatedAtMs ?? rightItem.createdAtMs ?? 0) -
      Number(leftItem.updatedAtMs ?? leftItem.createdAtMs ?? 0)
  );
}
...
export function savePatientMemory(patientId, userId, payload) {
  return addDoc(collection(db, COLLECTIONS.patientMemories), {
    ...
    imageUrl: payload.imageUrl?.trim() ?? '',
    imagePath: payload.imagePath?.trim() ?? '',
    voiceNoteUrl: payload.voiceNoteUrl?.trim() ?? '',
    voiceNotePath: payload.voiceNotePath?.trim() ?? '',
    createdAt: serverTimestamp(),
    createdAtMs: Date.now(),
  });
}

export function savePatientImportantPerson(patientId, userId, payload) {
  return addDoc(collection(db, COLLECTIONS.patientImportantPeople), {
    patientId,
    createdByUserId: userId,
...
export function updatePatientImportantPerson(personId, userId, payload) {
  return updateDoc(doc(db, COLLECTIONS.patientImportantPeople, personId), {
    name: payload.name?.trim() ?? '',
    relationship: payload.relationship?.trim() ?? '',
    description: payload.description?.trim() ?? '',
    imageUrl: payload.imageUrl?.trim() ?? '',
...
export function deletePatientImportantPerson(personId) {
  return deleteDoc(doc(db, COLLECTIONS.patientImportantPeople, personId));
}
